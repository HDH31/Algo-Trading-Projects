...
class ERC:
    ...
    def solve(self,precision,seed):
        ...
        w[:]=seed[:]
        iter,n,grad=0,len(w),list(range(3))
        while True:
            ...
            grad[0]=-err[iErr]
            grad[1]=w[iErr,0]*self.covar[iErr,iErr]/risk**2*(1-2*ctb[iErr]**2)+ self.covar[iErr,iErr]**.5*ctb[iErr]/risk
            grad[2]=self.covar[iErr,iErr]/risk**2*(1-2*ctb[iErr]**2)- ctb[iErr]*w[iErr,0]*(self.covar[iErr,iErr]**.5/risk)**3*(2-3*ctb[iErr]**2)
            """#5) Step size
            if grad[2]!=0:
                if grad[1]**2-4*grad[2]*grad[0] > 0:
